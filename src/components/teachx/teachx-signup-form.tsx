...
export function TeachXSignupForm() {
  ...
  async function onSubmit(e: FormEvent) {
    ...
    try {
      ...
      const dest = typeof data.redirectTo === "string" ? data.redirectTo : "/teachx/dashboard";
      router.push(dest);
      router.refresh();
    } catch {
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4">
      ...
      <p className="text-center text-sm text-slate-500">
        ...
        <Link href="/teachx/login" className="font-semibold text-blue-600 hover:text-blue-700">
          Log in
        </Link>
      </p>
    </form>
